/* ═══════════════════════════════════════════════
   ...
   Auth: Bearer token validated against env constant
   Data: Supabase REST API with service role key
════════════════════════════════════════════════ */
...
/* ─────────────────────────────────────────────
   NOTE ON KEYS:
   - SUPABASE_SERVICE_KEY bypasses RLS so the
     admin can read all rows. Keep this secret.
   - ADMIN_TOKEN is your own password to access
     this dashboard. Use a long random string.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function supabaseFetch(table, params = '') {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/${table}?${params}&order=submitted_at.desc`, {
      headers: {
